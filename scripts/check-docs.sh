...
for public_file in "$ROOT/README.md" "$ROOT"/site/*.html \
	"$ROOT"/site/docs/*.html "$ROOT"/docs/*.md \
	"$ROOT/hardware/w5500/README.md" \
	"$ROOT"/hardware/w5500/docs/*.md; do
	if grep -Eiq 'interview|résumé|resume' "$public_file"; then
		echo "Private career framing found in public documentation: $public_file" >&2
		exit 1
	fi
done
